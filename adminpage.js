...
function phase(){
    fetch('phase.php')
        .then(response => response.json())
        .then(data => {
            // Assuming there's an element with the id 'output' to display the result
            const outputElement = document.getElementById('output');

            if (data === 0) {
                outputElement.textContent = 'Voting phase';
            } else {
                outputElement.textContent = 'Result phase: ';
            }
        })
        .catch(error => {
            console.error('Error:', error);
...
const codeField = document.querySelector('#profile-popup h4:nth-child(4)');
let userData; // Declare userData here

profileButton.addEventListener('click', () => {
    // Make an AJAX request to fetch user data from the server
    fetch('getprofile1.php') // Replace with your server endpoint
        .then(response => response.json())
        .then(data => {
            userData = data; // Store the data in the userData variable
            codeField.textContent = `Election Code: ${data[0].Code}`;
